...
# 创建子菜单：SHU图标固定
shu_menu = SubMenu('SHU图标')
shu_menu.set_value_getter(config, lambda x: x['layout']['logo_shu'])
shu_menu.set_compare_method(lambda x, y: x == y)
root_menu.add(shu_menu)

# 创建菜单项：logo：启用
shu_enable_menu = MenuItem('启用')
shu_enable_menu._value = True
shu_enable_menu.set_procedure(config.enable_shu)
shu_menu.add(shu_enable_menu)

# 创建菜单项：logo：不启用
shu_disable_menu = MenuItem('不启用')
shu_disable_menu._value = False
shu_disable_menu.set_procedure(config.disable_shu)
shu_menu.add(shu_disable_menu)
...
# 设定默认logo
default_logo_menu = SubMenu('【新选项】设置默认 logo，机身无法匹配时将使用默认 logo（比如上海电影学院）')
